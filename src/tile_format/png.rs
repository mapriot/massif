...
///   1–3 → Fast, 4–7 → Default, 8–9 → Best
/// Omitting `compress` uses Fast (prioritises speed, consistent with the
/// no-compress WebP path).
pub fn encode_tile(rgb: &[u8], compress: Option<u8>) -> Result<Vec<u8>> {
    let level = match compress {
        None | Some(1..=3) => png::Compression::Fast,
        Some(4..=7) => png::Compression::Default,
