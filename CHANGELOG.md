...
- Dockerfile for building and running massif without a local GDAL or Rust install. Build once with `docker build -t massif .`, then `docker run --rm -v "$PWD:/data" massif …`.
